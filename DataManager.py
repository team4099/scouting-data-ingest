...
class DataManager:
    ...
    def validate_config(self):
        if "TBA-Key" not in self.config:
            ...
            return False
        elif requests.get("https://www.thebluealliance.com/api/v3/status",
                          headers={'X-TBA-Auth-Key': self.config['TBA-Key']}).status_code == 401:
            self.log.error("The key listed in the TBA-Key field is not valid. Please ensure the key is correct.")
            return False
        ...
        if "Google-Credentials" not in self.config:
            ...
            try:
                ...
                self.log.error("The file listed in the Google-Credentials Field is improper. See below for details.")
                self.log.error(e)
                return False
        ...
        try:
            ...
            self.log.error("Your Databse user name and/or password is not correct. Please verify them.")
        ...
        if requests.get(f"https://www.thebluealliance.com/api/v3/event/{self.config['Year']}{self.config['Event']}",
                        headers={'X-TBA-Auth-Key': self.config['TBA-Key']}).status_code == 404:
            self.log.error(
